...
class RESTpAPI
{
    ...
    {
        require_once RESTpAPIABSPATH . '../../app/libs/script/classes/PDOModel.php';
    ...
    public function setHttpHeaders($responseContentType = "", $statusCode = "")
    {
        ...
        if ($this->settings["allowOriginHeader"]) {
            header("Access-Control-Allow-Orgin: " . $this->settings["allowOriginURL"]);
    ...
    {
        $responseContentType = $_SERVER['HTTP_ACCEPT'];
    ...
    public function handleRequest($operationType, $data)
    {
        switch (strtolower($operationType)) {
            ...
            case "delete":
                ...
                break;
            case "query":
                return $this->dbQuery($data);
                break;
            case "droptable":
                return $this->dbDropTable($data);
                break;
            case "renametable":
                return $this->dbRenameTable($data);
                break;
            case "truncatetable":
                return $this->dbTruncateTable($data);
                break;
            case "primarykey":
                return $this->dbPrimaryKey($data);
                break;
            case "tables":
                return $this->dbAllDBTables($data);
                break;
            case "columns":
                return $this->dbGetColumns($data);
                break;
            case "jwtauth":
                return $this->dbJWTAuth($data);
                break;
        }
    }
    ...
    public function dbJWTAuth($data)
    {
        ...
        if ($pdoModelObj->totalRows > 0 && $verifyPassword) {
            ...
            try {
                ...
                $encoded = Firebase\JWT\JWT::encode($payload, $key);
                $this->message = $this->getLangData("success");
                $this->statusCode = 200;
            } catch (Exception $e) {
                return false;
    ...
    public function validateJWT()
    {
        ...
        if (isset($_SERVER["HTTP_AUTHORIZATION"])) {
            $jwt = trim(ltrim($_SERVER["HTTP_AUTHORIZATION"], "Bearer"));
        } else {
            $header = apache_request_headers();
            if (isset($header["Authorization"]))
                $jwt = trim(ltrim($header["Authorization"], "Bearer"));
        }
        if (!empty($jwt)) {
            $key = isset($this->settings["secretkey"]) ? $this->settings["secretkey"] : "";
            $algs = isset($this->settings["supportedAlgorithms"]) ? $this->settings["supportedAlgorithms"] : array('HS256');
            require_once RESTpAPIABSPATH . 'library/php-jwt-master/src/JWT.php';
            try {
                $decoded = Firebase\JWT\JWT::decode($jwt, $key, $algs);
                if ($decoded)
                    return true;
            } catch (Exception $e) {
                return false;
