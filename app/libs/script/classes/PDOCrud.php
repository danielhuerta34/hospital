...
class PDOCrud
{
    ...
    private function initializeSettings()
    {
        ...
        $this->fileUploadPath = PDOCrudABSPATH . $_ENV["UPLOAD_FOLDER"];
        $this->fileSavePath = PDOCrudABSPATH . $_ENV["DOWNLOAD_FOLDER"];
    ...
    public function fieldGroups($groupName, $fields = array())
    {
        ...
        if (is_array($fields) && count($fields) > 0) {
            ...
            foreach ($fields as $field) {
                $this->fieldBlockClass($field, "col-" . $colDiv);
            }
        }
        return $this;
    }
    ...
    public function formFieldValue($fieldName, $value, $fieldHidden = false, $tablename = "")
    {
        ...
        if ($fieldHidden) {
            $this->formFieldHide[$fieldName] = array(
                "fieldHidden" => $fieldHidden, "tablename" => $tablename,
                "value" => $value
            );
        }
        return $this;
    ...
    {
        $this->crudFilter[$filterName] = array(
    ...
    {
        $this->advSearchDataSource[$columnName] = array(
            "dataSource" => $dataSource,
            "key" => $key,
            "val" => $val,
            "bind" => $bind
    ...
    {
        $this->advSearchParam[$columnName] = array(
            "displayText" => $displayText,
    ...
    public function addChart($chartName, $chartType, $dataSource, $key, $val, $bind = "db", $param = array())
    {
        switch (strtolower($chartType)) {
            case "easypie":
                $this->addPlugin("jquery-easy-pie-chart");
                break;
            case "sparkline":
                $this->addPlugin("sparkline");
                break;
            case "google-chart":
                $this->addPlugin("google-chart");
                break;
            default:
                $this->addError("");
                return;
        ...
        if (isset($this->chart)) {
            ...
            foreach ($this->chart as $key => $val) {
                switch (strtolower($val["chartType"])) {
                    case "easypie":
                        $pluginName = "jquery-easy-pie-chart";
                        $key = "#" . $key;
                        break;
                    case "sparkline":
                        $pluginName = "sparkline";
                        $val["param"] = array();
                        $key = "#" . $key;
                        break;
                    case "google-chart":
                        $pluginName = "google-chart";
                        $val["param"] = array();
    ...
    public function setAjaxActions($elementName, $event, $callbackFunc, $returnValueElement = "", $otherElements = array())
    {
        ...
        $this->jsSettings["ajax_actions"][] = array(
            "element_name" => $elementName, "event" => $event,
    ...
    public function setLeftJoinJsActions($element, $formula, $event, $eventFields)
    {
        ...
        $field = rtrim($field, ", ");
        $this->enqueueHTMLContent($this->pdocrudView->renderLeftJoinJSFormula(
            $element,
            $formula,
            $event,
            $field
    ...
    private function formatFormData($values)
    {
        ...
        if (isset($this->formFieldHide)) {
            foreach ($this->formFieldHide as $field => $val) {
                ...
                if (isset($this->fieldFormula[$field])) {
                    $val = $this->pdoTableFormatObj->formatTableColOptions(
                        $field,
                        $this->fieldFormula[$field],
                        $val["value"],
                        $data[$tablename]
                    );
                } else
    ...
    private function dbSQL($data)
    {
        ...
        $this->setSettings("printBtn", false);
        $this->setSettings("pdfBtn", false);
        $this->setSettings("csvBtn", false);
        $this->setSettings("excelBtn", false);
    ...
    private function formatTableData($result)
    {
        ...
        if (is_array($this->bulkCrudUpdateCol) && count($this->bulkCrudUpdateCol)) {
            ...
            foreach ($this->bulkCrudUpdateCol as $col => $param) {
                ...
                $data[$col] = array("field" => $this->getHTMLElement($col . "[]", $param["fieldType"], $param["attr"],  array("{val}"),  $param["fieldData"], array("input-bulk-crud-update seleccion_" . $col)));
    ...
    private function renderFormData($data, $submitData = array(), $type = "insert")
    {
        ...
        if (isset($this->leftJoin))
            $output .= "<div class='table-responsive'>" . $this->leftJoin . "</div>";
    ...
    public function getRadioButtonField4($fieldName, $attr, $data, $fieldData, $fieldClass = array())
    {
        ...
        if (is_array($fieldData) && count($fieldData)) {
            ...
            foreach ($fieldData as $fieldsval) {
                ...
                $field .= " />";
                $field .= "<label class=\"form-check-label radio-inline $class\">";
                $field .= $fieldsval[1];
                $field .= "</label>";
                $field .= "</div>";
                $loopFields++;
    ...
    public function getLableField($lableText, $lableFor, $lableClass = array())
    {
        ...
        $lable_guion = strtr($lableText, ' ', '_');
        $lable = "<label for=\"$lableFor\" class=\"$class label_$lable_guion\">$lableText</label>";
        return $lable;
    }
    ...
    public function getSearchBox($columns, $data = array())
    {
        ...
        $search .= "</select></div>";
        $search .= "<div class=\"col-md-5 col-sm-10 col-10 pdo-table-search form-group no-padding-right\">";
        ...
        $search .= "</div>";
        $search .= "<div class=\"col-md-1 col-sm-1 col-1 pdo-search-cols no-padding\">";
        $search .= "<a href=\"javascript:;\" id=\"pdocrud_search_btn\" name=\"pdocrud_search_btn\" class=\"busqueda btn btn-primary pdocrud-form-control pdocrud-actions $class\" data-action=\"search\">";
